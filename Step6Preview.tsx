...
// FIX: Import MUSICAL_KEYS to be used with the keyIndex.
...
const generatePrompt = (data: FormData): string => {
    ...
    if (data.language !== 'Instrumental') {
        parts.push(`Any vocals should be in ${data.language}.`);
    } else {
        parts.push(`The track is purely instrumental.`);
    }
...
const Step6Preview: React.FC<PreviewProps> = ({ formData }) => {
    ...
    return (
        ...
        <StepWrapper title="Your Final Prompt" stepKey={6}>
            <div className="relative">
                <pre className="bg-gray-900 p-4 rounded-lg text-gray-300 whitespace-pre-wrap font-mono text-sm overflow-x-auto border border-gray-700">
                    <code>
                        {promptText}
                    </code>
                </pre>
