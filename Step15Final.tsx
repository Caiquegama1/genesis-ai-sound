...
// FIX: Import MUSICAL_KEYS to be used with the new keyIndex property.
...
const generatePrompt = (data: FormData): string => {
    ...
    // FIX: Use the keyIndex from form data to get the key string from MUSICAL_KEYS.
    const displayKey = MUSICAL_KEYS[data.keyIndex];
    ...
    if (data.startMode === 'AUDIO_REF') {
        parts.push(`Audio reference fidelity: ${data.audioRefFidelity}% (melody/harmony/feel).`);
    }

    parts.push(
        `[${finalGenre} - ${finalSubgenre}] song in ${displayKey} at ${data.bpm} BPM.`,
...
const Step15Final: React.FC<Pick<StepProps, 'formData' | 't'> & { handleCreateNew: () => void; }> = ({ formData, t, handleCreateNew }) => {
    ...
    return (
        <StepWrapper title={t('final.title')} stepKey={15}>
